...
This is done via a `download-download-gen-schemas.ts` script in the consuming service's `bin/` directory. Use the `SchemaTypeLoader` utility to download JSON Schemas, then convert to Zod with `json-schema-to-zod`.

```typescript
// In the consuming service's bin/download-download-gen-schemas.ts file
...
After running the `download-download-gen-schemas.ts` script, the consuming service will have a new file at `lib/handlers/src/__generated__/myServiceApiBase.ts`.
